...
(paper_agent_next) CLI.  v2 handles all its own config parsing, so we just
...
# Known engine-related env var names (without PAPER_AGENT_ prefix).
...
def request_to_cli_args(request: Any) -> list[str]:
    """Convert a TranslateRequest to paper_agent_next CLI arguments."""
...
def request_to_env(request: Any) -> dict[str, str]:
    """Build env dict with PAPER_AGENT_ prefixed vars for the v2 subprocess.

    v2's ConfigManager reads env vars with a ``PAPER_AGENT_`` prefix.  This
    ...
    setting ``PAPER_AGENT_{ENGINE}_MODEL``.
    """
    env: dict[str, str] = {}
    data = dataclasses.asdict(request)
    envs = data.get("envs") or {}

    # Map v1 env vars from request.envs → PAPER_AGENT_ prefix
    for key, value in envs.items():
        env[f"PAPER_AGENT_{key.upper()}"] = str(value)

    # Forward relevant vars from os.environ (if not already set)
    for key in _ENGINE_ENV_NAMES:
        v2_key = f"PAPER_AGENT_{key}"
        if v2_key not in env and key in os.environ:
            env[v2_key] = os.environ[key]

    # Handle service:model → PAPER_AGENT_{ENGINE}_MODEL
    service_raw = data.get("service", "google")
    service, model = _split_service_model(service_raw)
    if model:
        engine_type = SERVICE_NAME_MAP.get(service.lower())
        if engine_type:
            model_env = f"PAPER_AGENT_{engine_type.upper()}_MODEL"
            env[model_env] = model
